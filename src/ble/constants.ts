...
/**
 * `requestDevice` options — a narrow net.
 *
 ...
 *      "BIKE 42"). Name-based discovery is what's proven in the field — the 128-bit UART UUID
 *      isn't reliably in the advertisement, so a service filter alone can miss the bike. This is
 *      the route we trust.
 *   2. `services: [ICG_SERVICE]` — a backstop if a unit's name differs but it does advertise the
